...
static BMVert *bm_vert_create_co(BMesh *bm, float x, float y, float z)
{
	...
	return BM_vert_create(bm, co, NULL);
}
...
#define CHECK_SLOT_ELEM_COUNTS(bm_, op_, slot_name_,				\
	...
	do {															\
		...
		int totvert = 0, totedge = 0, tottriangle = 0;				\
																	\
		assert(BMO_slot_exists(op_, slot_name_));					\
																	\
		BMO_ITER (ele, &oiter, (bm_), op_, slot_name_, BM_ALL) {	\
...
static int bmo_slot_buffer_contains(BMesh *bm, BMOperator *op,
									const char *slot_name, void *key)
{
	BMOIter oiter;
	BMElemF *ele;

	assert(BMO_slot_exists(op, slot_name));
	BMO_ITER (ele, &oiter, bm, op, slot_name, BM_ALL) {
		if (ele == key)
			return TRUE;
	}
	return FALSE;
...
static void test_convex_hull_tetrahedron(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 4, 6, 4);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);
...
static void test_convex_hull_interior_elems(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 4, 6, 4);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 3, 3, 1);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 3, 3, 1);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);

	for (i = 0; i < 3; i++) {
		assert(bmo_slot_buffer_contains(bm, &op, "interior_geom", v[4 + i]));
		assert(bmo_slot_buffer_contains(bm, &op, "interior_geom", e[i]));
		assert(bmo_slot_buffer_contains(bm, &op, "unused_geom", v[4 + i]));
		assert(bmo_slot_buffer_contains(bm, &op, "unused_geom", e[i]));
	}
	assert(bmo_slot_buffer_contains(bm, &op, "interior_geom", f[0]));
	assert(bmo_slot_buffer_contains(bm, &op, "unused_geom", f[0]));
...
static void test_convex_hull_unused_edge_and_face(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 5, 9, 6);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 0, 1, 1);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 1, 1);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);
...
static void test_convex_hull_interior_but_used(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 4, 6, 4);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 2, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);

	assert(bmo_slot_buffer_contains(bm, &op, "interior_geom", v[4]));
	assert(bmo_slot_buffer_contains(bm, &op, "interior_geom", v[5]));
...
static void test_convex_hull_single_hole(void)
{
	 ...
	 * were already created, not in geomout. Is this good behavior? */
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 6, 3 + 3 + 3 + 3, 2 * 3);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 1);

	//assert(holes_geom_not_in_other_slots_check(bm, &op));

	assert(bmo_slot_buffer_contains(bm, &op, "holes_geom", f[2]));
...
static void test_convex_hull_one_edge(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 3, 3, 1);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);
...
static void test_convex_hull_one_triangle_use_existing(void)
{
	...
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "geomout", 3, 3, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "interior_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "unused_geom", 0, 0, 0);
	CHECK_SLOT_ELEM_COUNTS(bm, &op, "holes_geom", 0, 0, 0);
